...
curl -OL https://github.com/DNSCrypt/dnscrypt-proxy/releases/download/$LATEST/dnscrypt-proxy-linux_x86_64-${LATEST}.tar.gz \
     -OL https://github.com/DNSCrypt/dnscrypt-proxy/releases/download/$LATEST/dnscrypt-proxy-linux_x86_64-${LATEST}.tar.gz.minisig
```
...
```sh copy
minisign \
    -Vm dnscrypt-proxy-linux_x86_64-${LATEST}.tar.gz \
...
```sh copy
tar xzf dnscrypt-proxy-linux_x86_64-${LATEST}.tar.gz
```
...
```sh copy
$SU cp linux-x86_64/dnscrypt-proxy /usr/bin/
$SU cp linux-x86_64/example-dnscrypt-proxy.toml /etc/dnscrypt-proxy/dnscrypt-proxy.toml
```
...
$SU rm -rf /tmp/dnscrypt-proxy-linux_x86_64-${LATEST}* /tmp/linux-x86_64
```
</Steps>
...
<FAQBox title="Anonimized DNS">
If you want to anonymize your DNS queries, you can use the `anonymized_dns`
option. This will route your DNS queries through anon relays. Read more about
this [here](https://github.com/DNSCrypt/dnscrypt-proxy/wiki/Anonymized-DNS).
...
You need to disable the servers you use as relays `via=[...]` in the
`disabled_server_names`. These are just an example, you need to find the
appropriate servers for you.

```toml copy filename="/etc/dnscrypt-proxy/dnscrypt-proxy.toml" {3,6,9,12,15,18,21,24,27,31-33,37}
[...]
# Use servers reachable over IPv4
ipv4_servers = true
...
disabled_server_names = ['cs-barcelona', 'cs-fr', 'portland-ipv4']
[...]
[anonimyzed_dns]

routes = [
    { server_name='*', via=['anon-cs-barcelona', 'anon-cs-fr', 'dnscry.pt-anon-portland-ipv4'] }
]
[...]
## Skip resolvers incompatible with anonymization instead of using them directly
