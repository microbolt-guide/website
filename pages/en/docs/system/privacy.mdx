...
[DNSCrypt](https://dnscrypt.info) is a protocol that authenticates
...
```sh copy
cd /tmp
LATEST=$(wget -qO- https://api.github.com/repos/DNSCrypt/dnscrypt-proxy/releases/latest | grep "tag_name" | cut -d '"' -f 4 | sed 's/^v//')
wget https://github.com/DNSCrypt/dnscrypt-proxy/releases/download/$LATEST/dnscrypt-proxy-linux_x86_64-${LATEST}.tar.gz \
     https://github.com/DNSCrypt/dnscrypt-proxy/releases/download/$LATEST/dnscrypt-proxy-linux_x86_64-${LATEST}.tar.gz.minisig
```
...
```sh copy
$SU tar xzf dnscrypt-proxy-linux_x86_64-${LATEST}.tar.gz
```
