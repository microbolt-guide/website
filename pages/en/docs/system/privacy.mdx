...
[DNSCrypt]: https://www.dnscrypt.info
[official documentation]: https://github.com/DNSCrypt/dnscrypt-proxy/wiki/Configuration
[here]: https://github.com/DNSCrypt/dnscrypt-proxy/wiki/Anonymized-DNS
[available relays]: https://github.com/DNSCrypt/dnscrypt-resolvers/blob/master/v3/relays.md
...
[iplocation.net], your physical address can be determined quite accurately.
Especially with Lightning, your IP address would be widely used. We need to make
sure that you keep your privacy.

## Tor Project

We use Tor, a free software built by the [Tor Project]. It allows you to
anonymize internet traffic by routing it through a network of nodes, hiding your
...
```sh filename="/etc/tor/torrc" {7-8,12-13,17-20}
[...]
...
SOCKSPort 9050
SOCKSPort unix:/run/tor/socks.sock WorldWritable
[...]
## The port on which Tor will listen for local connections from Tor
## controller applications, as documented in control-spec.txt.
ControlPort 9051
ControlPort unix:/run/tor/control.sock GroupWritable RelaxDirModeCheck
...
tcp        0      0 127.0.0.1:9051          0.0.0.0:*               LISTEN      6432/tor
```
...
### `curl`, `git` and `npm` through Tor

To download files with `curl`, `git`, and `npm` through Tor, you can use the
`*_PROXY` environment variable. This is useful for downloading files from the
internet without revealing your IP address.

<Callout type="warning">
This approach may not work for all applications. Always test after configuring
the proxy to ensure proper functionality. We use `curl` because `wget` does not
support the `socks5h` proxy. Everything in this guide has been tested and works
with this Tor configuration.
</Callout>
...
[I2P] is a universal anonymous network layer. All communications over I2P are
anonymous and end-to-end encrypted, participants don't reveal their real IP
addresses. I2P allows people from all around the world to communicate and share
...
We are to use [i2pd] (I2P Daemon), a full-featured C++ implementation of the I2P
client, as a Tor network complement.
...
<Callout type="info">
You can learn more about the configuration options in the
[official documentation]
</Callout>

<FAQBox title="Anonymized DNS">
If you want to anonymize your DNS queries, you can use the `[anonymized_dns]`
option. This will route your DNS queries through anonymous relays. More [here].
