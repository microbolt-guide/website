...
[here]: https://alpinelinux.org/cloud
[Ansible]: /ansible
...
[Alpine Linux] is available for most hardware platforms. There is so little need
for hardware that can run in a router.
...
<Callout type="info">
Microbolt has currently been tested using **Alpine Linux {VERSION}** on a
`x86_64` architecture, it has not been tested on other distributions,
...
| Usually      | PC         | VM        | Raspberry Pi \{3,4,5\} |
| :----------- | :--------: | :-------: | :--------------------: |
| version      | `standard` | `virtual` | `raspberry pi`         |
| architecture | `x86_64`   | `x86_64`  | `aarch64`              |

<Callout type="info">
There's also official cloud images [here], use at your own risk. Best suited for
the [Ansible] guide.
</Callout>
...
```
    ...
    <Tabs.Tab>
For example, there is the [Rufus] program.
...
❌ **Diskless mode** (not suitable for a bitcoin full-node)

You'll `boot from read-only medium` such as the installation CD, a USB drive, or
a Compact Flash card.

When you use Alpine in this mode, you need to use [Alpine Local Backup] (lbu) to
save your modifications between reboots. That requires some writable medium,
usually removable. (If your boot medium is, for example, a USB drive, you can
save modifications there; you don't need a separate partition or drive.) See
also [Local APK cache].
...
![Alpine Linux installation modes](/images/system/alpine-modes.webp)
</Callout>
...
<FAQBox title="Secondary disk">
If you have a secondary disk, you can use it to store your data. You can use the
`data` mode alongside `sys` mode to install `/var` on the secondary disk.

<Steps>
{<h3>Setup environment variables</h3>}
As shown above, these variables are used to customize the disk installation.

{<h3>Secondary disk installation</h3>}
```sh copy
DEFAULT_DISK=none setup-disk -q
```

Which disk would you like to use: `sdX` (choose your secondary hard disk, usually sdb)
...
<FAQBox title="Swapfile">
If you exported `SWAP_SIZE=0` duriong installation you can create a `swapfile`
on next boot

<Steps>
{<h3>Allocate swapfile</h3>}
...
{<h3>Start and enable swapfile on boot</h3>}
