...
export const name = "Samourai Dojo"
export const user = "samourai"

[security section]: ../system/security#reverse-proxy
[release notes]: https://github.com/Dojo-Open-Source-Project/samourai-dojo/releases
[MariaDB]: https://mariadb.org
[{name}]: ./dojo

# Samourai Dojo

[{name}](https://github.com/Dojo-Open-Source-Project/samourai-dojo) is the
backing server for Samourai Wallet. Provides HD account & loose addresses
...
```sh copy
curl \
    --proxy socks5h://localhost/run/tor/socks.sock \
...
```sh copy
# Fix hidden services
sed "s#\`http://\${fs.readFileSync('/var/lib/tor/hsv3dojo/hostname', 'utf8').trim()}\`#process.env.DOJO_HSV3#g" \
...
service on the Microbolt and accessing the {name} with the Tor browser from any
device.
...
TOR_SOCKS_PORT=9050
NET_DOJO_MYSQL_IPV4=/run/mysqld/mysqld.sock
...
* Make the configuration file readable only by the {user} user

```sh copy
$SU chmod 600 /etc/samourai-dojo/mainnet.env
```

<FAQBox title="Remove ban Bitcoin client Knots">
If you are using the Bitcoin client Knots, you need to remove the ban check
before starting the {name}. Otherwise, the Dojo will not be able to
...
/var/log/samourai-dojo/*.log {
    ...
    sharedscripts
    postrotate
        kill -HUP `cat /run/samourai-dojo/samourai-dojo.pid`
    endscript
}
```
