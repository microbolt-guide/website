...
### Add {user} user to the bitcoin group
...
We'll download, verify and install {name}.
...
```sh copy
curl -OL https://github.com/lightningnetwork/lnd/releases/download/v$VERSION/lnd-linux-${ARCH}-v$VERSION.tar.gz \
...
```sh copy
grep lnd-linux-${ARCH}-v$VERSION.tar.gz manifest-v$VERSION.txt | sha256sum -c
```

```sh filename="output"
lnd-linux-${ARCH}-v${VERSION}.tar.gz: OK
```
...
* Check that the date of the timestamp is close to the [release notes] date of
the {name} binary.
...
```sh copy
$SU tar xzf lnd-linux-${ARCH}-v$VERSION.tar.gz -C /usr/bin --strip-components=1
```
...
```sh filename="output"
lnd version ${VERSION} commit=v${VERSION}
```
</Steps>
