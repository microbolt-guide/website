...
[...]
wallet-unlock-password-file=/etc/lnd/main-passwd.txt
[...]
...
: ${LND_PASSWDFILE:=/etc/lnd/main-passwd.txt}
...
```sh copy
$SU $EDITOR /etc/lnd/main-passwd.txt
```
...
stored in `main-passwd.txt`). To create a a new wallet, select `n` when asked if
you have an existing cipher seed. Just press enter if asked about an additional
