...
```sh copy
ln -s /var/lib/lightningd "$HOME/.lightningd"
```
</Steps>
...
```sh copy
MAN=0
sed -e '/^\s*$(MKDIR_P) \$(DESTDIR)\$(man[0-9]dir)/ s/^/#/' \
    -e '/^\s*$(MKDIR_P) \$(DESTDIR)\$(docdir)/ s/^/#/' \
    -e '/^datadir = / s/^/#/' \
    -e '/^docdir = / s/^/#/' \
    -e '/^mandir = / s/^/#/' \
    -e '/^man[0-9]dir = / s/^/#/' Makefile > _
mv -f _ Makefile
```
</Callout>
...
/var/log/lightningd/*.log {
    ...
    sharedscripts
    postrotate
        killall -HUP `cat /run/lightningd/lightningd.pid`
    endscript
}
```
