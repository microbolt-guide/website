...
![GIF of Linus Torvalds saying "fuck you ordinals"](/img/bitcoin/client/urinals.gif)
...
![Screenshot of OpenTimestamps verification for Bitcoin Core checksum file](/img/bitcoin/client/bitcoin-ots-check.webp)
...
[ ! "$password" ] && { 
    ...
    $SU sed "s/^#*rpcauth=.*$/rpcauth=${username}:${salt=$(openssl rand -hex 16)}\$$(\
            printf "%s" "${password}" | \
            openssl dgst -sha256 -hmac "$salt" | \
            awk '{print $2}' \
        )/" /etc/bitcoind/main.conf | $SU tee _; \
    unset salt; \
...
```nginx filename="/etc/bitcoind/main.conf" {2,4,6,8,10,12,14-15,17,19,21,23,25,27,29,31,33-34,36,38,40,42}
[...]
assumevalid=0
[...]
blockfilterindex=1
...
### Use bitcoind and bitcoin-cli with satoshi user
...
/var/log/bitcoind/main/*.log {
    ...
    sharedscripts
    postrotate
        kill -HUP `cat /run/bitcoind/main.pid`
    endscript
}
```
...
![Cover of the book 'Mastering Bitcoin'](/img/bitcoin/client/mastering-bitcoin.webp)
...
to locally verify the timestamp of the binaries checksums file.
