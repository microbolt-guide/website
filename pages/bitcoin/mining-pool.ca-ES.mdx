...
#### Instal·la dependències i construeix
...
```sh copy
$SU install -D -m 0660 -o public-pool -g public-pool ./.env.example /etc/public-pool/public-pool.conf
```
...
```sh copy
$SU $EDITOR /etc/public-pool/public-pool.conf
```

```sh filename="/etc/public-pool/public-pool.conf" {2,4,6,8-9,11}
[...]
...
: ${PUBLICPOOL_CONFIGFILE:=/etc/public-pool/public-pool.conf}
...
command_args="--env-file ${PUBLICPOOL_CONFIGFILE}
              ${PUBLICPOOL_OPTS}"
...
checkconfig() {
    if ! grep -qs '^BITCOIN_RPC_COOKIEFILE=' "${PUBLICPOOL_CONFIGFILE}"
    then
        eerror ""
