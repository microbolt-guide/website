...
PUBLICPOOL_PIDFILE="/run/${SVCNAME}.pid"
...
start_pre() {
    checkpath --file      --mode 0660 --owner "${PUBLICPOOL_USER}:${PUBLICPOOL_GROUP}" "${PUBLICPOOL_CONFIGFILE}"
    checkpath --directory --mode 0750 --owner "${PUBLICPOOL_USER}:${PUBLICPOOL_GROUP}" "${PUBLICPOOL_DATADIR}"
    checkpath --directory --mode 0755 --owner "${PUBLICPOOL_USER}:${PUBLICPOOL_GROUP}" "${PUBLICPOOL_LOGDIR}"
    checkpath --directory --mode 0755 --owner "${PUBLICPOOL_USER}:${PUBLICPOOL_GROUP}" "${PUBLICPOOL_WEBUI}"
    checkconfig
}

start_post() {
    chown -R "${PUBLICPOOL_USER}:${PUBLICPOOL_GROUP}" "${PUBLICPOOL_WEBUI}"
}
...
stop() {
    ...
    pkill -TERM -P "$(pgrep -P $(cat ${PUBLICPOOL_PIDFILE}))"
    start-stop-daemon \
        --stop \
        --pidfile="${PUBLICPOOL_PIDFILE}" \
...
/var/log/public-pool/*.log {
    ...
    sharedscripts
    postrotate
        killall -HUP `cat /run/public-pool.pid`
    endscript
}
```
