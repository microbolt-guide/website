...
### Configura, corregiu i instal·la

* Pegat [gh pr](https://github.com/apotdevin/thunderhub/pull/612)
...
* Redueix l'espai ocupat per ThunderHub
...
* Edita el fitxer `thubConfig.yaml`
...
* Amb el [navegador Tor](https://www.torproject.org), pots accedir a aquesta
adreça onion des de qualsevol dispositiu. Tingues en compte que aquest accés no
està protegit per contrasenya i no s'hauria de compartir amplament.
</FAQBox>
...
: ${THUNDERHUB_PIDFILE:=${THUNDERHUB_PIDDIR}/${SVCNAME}.pid}
...
start_pre() {
    checkpath --file      --mode 0660 --owner "${THUNDERHUB_USER}:${THUNDERHUB_GROUP}" "${THUNDERHUB_CONFIGFILE}"
    checkpath --directory --mode 0750 --owner "${THUNDERHUB_USER}:${THUNDERHUB_GROUP}" "${THUNDERHUB_DATADIR}"
    checkpath --directory --mode 0755 --owner "${THUNDERHUB_USER}:${THUNDERHUB_GROUP}" "${THUNDERHUB_LOGDIR}"
    checkpath --directory --mode 0755 --owner "${THUNDERHUB_USER}:${THUNDERHUB_GROUP}" "${THUNDERHUB_PIDDIR}"
    checkconfig
}
