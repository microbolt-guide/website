...
Hem configurat [LND](https://github.com/lightnetwork/lnd), la xarxa Lightning
Daemon de [Lightning Labs](https://lightning.Engineering).
...
```sh copy
$SU install -m 0755 -o root -g root -t /usr/bin lnd-linux-amd64-v$VERSION/*
```
...
: ${LND_PIDDIR:=/run/lnd}
: ${LND_PIDFILE:=${LND_PIDDIR}/${SVCNAME}.pid}
...
start_pre() {
    checkpath --file      --mode 0660 --owner "${LND_USER}:${LND_GROUP}" "${LND_CONFIGFILE}"
    checkpath --file      --mode 0600 --owner "${LND_USER}:${LND_GROUP}" "${LND_PASSWDFILE}"
    checkpath --directory --mode 0750 --owner "${LND_USER}:${LND_GROUP}" "${LND_DATADIR}"
    checkpath --directory --mode 0755 --owner "${LND_USER}:${LND_GROUP}" "${LND_LOGDIR}"
    checkpath --directory --mode 0755 --owner "${LND_USER}:${LND_GROUP}" "${LND_PIDDIR}"
