...
<Steps>
### Definiu quina **editor de text** i **eina d'escalada de privilegis** voleu

Afegiu en un fitxer de configuració de perfil, substituïu `<...>` per la vostra
pròpia elecció
...
<Tabs items={['doas', 'sudo', 'ssu']}>
    <Tabs.Tab>
```sh copy
sed 's/# permit persist :wheel/permit persist :wheel/' /etc/doas.conf > _
