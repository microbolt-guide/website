...
* Assigna el fitxer d'intercanvi, si cal

```sh copy
[ "$SWAP_SIZE" == 0 ] && {
    fallocate -l 4GiB /var/swapfile
...
* Posa el fitxer d'intercanvi a l'arrencada, si cal
...
```
    ...
    <Tabs.Tab>
* Configura la interfície d'Internet
...
```sh
mydev=/dev/sdY #Normalment /dev/sdb
```
...
* Elimina possibles línies duplicades

```sh copy_
awk '!x[$1]++ && ! /^[[:blank:]]*$/' /etc/apk/repositories > _
mv -f _ /etc/apk/repositories
```
