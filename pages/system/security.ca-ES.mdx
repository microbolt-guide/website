...
<Steps>
{<h3>Creeu un nou parell de claus público/privada</h3>}
...
{<h3>Ara s'ha de copiar la clau pública a l'ordinador</h3>}
...
```sh copy
$SU mkdir -p /etc/nginx/streams-available /etc/nginx/streams-enabled
```
